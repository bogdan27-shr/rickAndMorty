import {createLocations} from './creaeteLocations.js';
...
export function locationsPage () {
  const req = async (url) => {
    ...
    req(`https://rickandmortyapi.com/api/location/?page=1`).then((cards) => {
      let cont = document.querySelector(`.contForCard`);
      cont.innerHTML = ``;
  
      cards.results.forEach((el) => {
        console.log(el);
        cont.append(createLocations(el));
      });
    }).catch(() => {
      ...
      if(currentPage >= 1 && currentPage <= 48){
        if(target.dataset.current === `next`){
          if(currentPage <= 6){
            ...
            req(`https://rickandmortyapi.com/api/location/?page=${currentPage}`).then((cards) => {
              let cont = document.querySelector(`.contForCard`);
              cont.innerHTML = ``;
